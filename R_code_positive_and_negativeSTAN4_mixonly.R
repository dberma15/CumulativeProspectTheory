...
translatedModel<-stanc(file="CPT_hierarchical_model_mixed_framework2.stan",model_name='CPTbehavioral')
...
prospects_b <- as.matrix(read.table("modeldatagamble17.txt"), fill=T) #gamble option (a gamble choice is a 1)
prospects_a <- as.matrix(read.table("modeldatasure17.txt"), fill=T) #sure option (a sure choice is a 0)
...
rawdata <- as.matrix(read.table("modeldatachoice17.txt"), fill=T)
items_n <- as.matrix(read.table("realpersonntrial17.txt"))
...
uppercovarmatrix<-lowercovarmatrix+6*subjectsperrun-1 #end of covariance terms of interest
covariancematrix<-matrix(c(0),1,6) #initializes an empty 1x6 matrix used to stored covariance terms
...
beta_upper=matrix(1,totalsubjects,1)
beta_lower=matrix(0,totalsubjects,1)
gamma_upper=matrix(1,totalsubjects,1)
gamma_lower=matrix(0,totalsubjects,1)
delta_upper=matrix(1,totalsubjects,1)
delta_lower=matrix(0,totalsubjects,1)

lambda_upper=matrix(5,totalsubjects,1)
lambda_lower=matrix(1,totalsubjects,1)
...
realValues = matrix(nrow=6, ncol=subsetOfSubjects)
realValues[1,]=t(as.matrix(read.table("alpha.txt"), fill=T))[1:subsetOfSubjects]
realValues[2,]=t(as.matrix(read.table("beta.txt"), fill=T))[1:subsetOfSubjects]
realValues[3,]=t(as.matrix(read.table("gamma.txt"), fill=T))[1:subsetOfSubjects]
realValues[4,]=t(as.matrix(read.table("delta.txt"), fill=T))[1:subsetOfSubjects]
realValues[5,]=t(as.matrix(read.table("lambda.txt"), fill=T))[1:subsetOfSubjects]
realValues[6,]=t(as.matrix(read.table("luce.txt"), fill=T))[1:subsetOfSubjects]
...
for(jjj in  1:subsetOfSubjects){
  ...
  items_n_neg=items_n[2,jjj]
  items_n_mix=items_n[3,jjj]
  rawdata_sub=rawdata[(items_n_neg+1):items_n_mix,jjj]
  repeatsize=1
  prospects_a_sub.mix=prospects_a_sub[(items_n_neg+1):items_n_mix,]
  prospects_b_sub.mix=prospects_b_sub[(items_n_neg+1):items_n_mix,]
  
  
  ax=abs(prospects_a_sub.mix[,1])^realValues[1,jjj]
  ay=-realValues[5,jjj]*abs(prospects_a_sub.mix[,3])^realValues[2,jjj]
  wax=prospects_a_sub.mix[,2]^realValues[3,jjj]/(prospects_a_sub.mix[,2]^realValues[3,jjj]+(1-prospects_a_sub.mix[,2])^realValues[3,jjj])^(1/realValues[3,jjj])
  way=prospects_a_sub.mix[,4]^realValues[4,jjj]/(prospects_a_sub.mix[,4]^realValues[4,jjj]+(1-prospects_a_sub.mix[,4])^realValues[4,jjj])^(1/realValues[4,jjj])
  vfa=ax*wax+ay*way
  
  bx=abs(prospects_b_sub.mix[,1])^realValues[1,jjj]
  by=-realValues[5,jjj]*abs(prospects_b_sub.mix[,3])^realValues[2,jjj]
  wbx=prospects_b_sub.mix[,2]^realValues[3,jjj]/(prospects_b_sub.mix[,2]^realValues[3,jjj]+(1-prospects_b_sub.mix[,2])^realValues[3,jjj])^(1/realValues[3,jjj])
  wby=prospects_b_sub.mix[,4]^realValues[4,jjj]/(prospects_b_sub.mix[,4]^realValues[4,jjj]+(1-prospects_b_sub.mix[,4])^realValues[4,jjj])^(1/realValues[4,jjj])
  vfb=bx*wbx+by*wby
  #vf=1/(1+exp(realValues[4,jjj]*((way*ay)+(wax*ax)-(wby*by)-(wbx*bx))))
  vf=realValues[3,jjj]*(vfb-vfa)
  
  items_n_mix=items_n[3,jjj]-items_n[2,jjj]
  #vfpos=c(vf)#,ay,bx,by)
  #rawdata_sub=wax
  ...
  player_dat  = list("prospects_a_mix"=prospects_a_sub.mix,
                     "prospects_b_mix"=prospects_b_sub.mix,
                     "N"=N,
                     "rawdata"=rawdata_sub, 
                     "items_n_mix"=items_n_mix,
                     "alpha_upper"=beta_upper[jjj],"alpha_lower"=beta_lower[jjj],
                     "beta_upper"=beta_upper[jjj],"beta_lower"=beta_lower[jjj],
                     "gamma_upper"=delta_upper[jjj],"gamma_lower"=delta_lower[jjj],
                     "delta_upper"=delta_upper[jjj],"delta_lower"=delta_lower[jjj],
                     "lambda_upper"=lambda_upper[jjj],"lambda_lower"=lambda_lower[jjj],
  ...
  parameters = c("alpha","beta", "gamma", "delta",'lambda', 'luce')
...
# .....lambda...
# .....luce.....
estimates<-matrix(nrow=6, ncol=subsetOfSubjects)
estimates[1,]<-alpha[2:(subsetOfSubjects+1),1]
estimates[2,]<-beta[2:(subsetOfSubjects+1),1]
estimates[3,]<-gamma[2:(subsetOfSubjects+1),1]
estimates[4,]<-delta[2:(subsetOfSubjects+1),1]
estimates[5,]<-lambda[2:(subsetOfSubjects+1),1]
estimates[6,]<-luce[2:(subsetOfSubjects+1),1]
...
for (j in 1:subsetOfSubjects){
  invCovarianceMatrix[[j]]<-solve(covariancematrix[(2+(6*(j-1))):(1+(6*j)),1:6])
  colnames(invCovarianceMatrix[[j]])<-labelsofCovarianceMat
  rownames(invCovarianceMatrix[[j]])<-labelsofCovarianceMat
  invVarianceMatrix[[j]]<-solve(diag(6)*covariancematrix[(2+(6*(j-1))):(1+(6*j)),1:6])
...
reducedChiSquare<-sum(chiSquareElement)/(2*subsetOfSubjects)
reducedChiSquare
chiSquareMatrix_Variance<-matrix(Reduce('+', chiSquareElement_Variance),6,6)
colnames(chiSquareMatrix_Variance)<-labelsofCovarianceMat
rownames(chiSquareMatrix_Variance)<-labelsofCovarianceMat
reducedChiSquare_Variance<-chiSquareMatrix_Variance%*%matrix(1,6,1)/(subsetOfSubjects-1)
...
write.table(covariancematrix,"C:\\Users\\Daniel\\Documents\\R\\Rstan_covariancemat_dataset17_mix.xls",sep="\t")
write.table(invCovarianceMatrix,"C:\\Users\\Daniel\\Documents\\R\\Rstan_invcovariancemat_dataset17_mix.xls",sep="\t")
write.table(alpha,"C:\\Users\\Daniel\\Documents\\R\\Rstan_alphaestimate_dataset17_mix.xls",sep="\t")
write.table(beta,"C:\\Users\\Daniel\\Documents\\R\\Rstan_betaestimate_dataset17_mix.xls",sep="\t")
write.table(delta,"C:\\Users\\Daniel\\Documents\\R\\Rstan_deltaestimate_dataset17_mix.xls",sep="\t")
write.table(gamma,"C:\\Users\\Daniel\\Documents\\R\\Rstan_gammaestimate_dataset17_mix.xls",sep="\t")
write.table(lambda,"C:\\Users\\Daniel\\Documents\\R\\Rstan_lambdaestimate_dataset17_mix.xls",sep="\t")
write.table(luce,"C:\\Users\\Daniel\\Documents\\R\\Rstan_luceestimate_dataset17_mix.xls",sep="\t")
allChiSquares<-matrix(c(chiSquareMatrix_Variance%*%matrix(1,6,1), sum(chiSquareElement)),7,1)
degreesOfFreedom<-rbind(matrix((subsetOfSubjects-1),6,1), 6*subsetOfSubjects)
...
write.table(allChiSquares,"C:\\Users\\Daniel\\Documents\\R\\Rstan_AllChiSquaresValues_dataset17_mix.xls",sep="\t")
