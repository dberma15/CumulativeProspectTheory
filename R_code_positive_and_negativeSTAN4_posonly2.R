...
#             100 130 124 80
#
#   knownalpha, knowngamma, knownluce - These are the known values of the parameters alpha, gamma, and luce where alpha=(0,1), gamma=(0,1), luce=(.5,1.5)
#                Three text files must be listed in these variables
#
...
subsetOfSubjects<-5 #The total number of subjects being run in the analysis (must be less than or equal to totalsubjects). 
totalsubjects<-subsetOfSubjects-firstSubject+1 #total number of subjects stored in the data sets
numberOfParameters=3 #number of parameters being estimated (3, alpha, gamma, luce)
...
#   
  ...
  parametersTemp<-c('alpha', 'gamma','luce') #order of parameters
  ...
  #Stores all variable estimates and basic statistics about them
...
estimates[2,]<-gamma[2:(totalsubjects+1),1]
estimates[3,]<-luce[2:(totalsubjects+1),1]
...
if (testingKnownData==1){
  ...
  realValues[2,]=t(as.matrix(read.table(knownGamma), fill=T))[firstSubject:subsetOfSubjects]
  realValues[3,]=t(as.matrix(read.table(knownLuce), fill=T))[firstSubject:subsetOfSubjects]
