...
translatedModel<-stanc(file="CPT_hierarchical_model_neg.stan",model_name='CPTbehavioral')
...
uppercovarmatrix<-lowercovarmatrix+4*subjectsperrun-1 #end of covariance terms of interest
covariancematrix<-matrix(c(0),1,4) #initializes an empty 1x6 matrix used to stored covariance terms
...
realValues = matrix(nrow=4, ncol=subsetOfSubjects)
realValues[1,]=t(as.matrix(read.table("beta.txt"), fill=T))[1:subsetOfSubjects]
realValues[2,]=t(as.matrix(read.table("delta.txt"), fill=T))[1:subsetOfSubjects]
realValues[3,]=t(as.matrix(read.table("lambda.txt"), fill=T))[1:subsetOfSubjects]
realValues[4,]=t(as.matrix(read.table("luce.txt"), fill=T))[1:subsetOfSubjects]
...
for(jjj in  1:subsetOfSubjects){
  ...
  rawdata_sub=rawdata[(items_n_pos+1):items_n_neg,jjj]
  repeatsize=1
  prospects_a_sub.neg=prospects_a_sub[(items_n_pos+1):items_n_neg,]
  prospects_b_sub.neg=prospects_b_sub[(items_n_pos+1):items_n_neg,]
  
  
  ax=-realValues[3,jjj]*abs(prospects_a_sub.neg[,1])^realValues[1,jjj]
  ay=-realValues[3,jjj]*abs(prospects_a_sub.neg[,3])^realValues[1,jjj]
  wax=prospects_a_sub.neg[,2]^realValues[2,jjj]/(prospects_a_sub.neg[,2]^realValues[2,jjj]+(1-prospects_a_sub.neg[,2])^realValues[2,jjj])^(1/realValues[2,jjj])
  way=prospects_a_sub.neg[,4]^realValues[2,jjj]/(prospects_a_sub.neg[,4]^realValues[2,jjj]+(1-prospects_a_sub.neg[,4])^realValues[2,jjj])^(1/realValues[2,jjj])
  #vfa=ax*wax+ay*way
  
  bx=-realValues[3,jjj]*abs(prospects_b_sub.neg[,1])^realValues[1,jjj]
  by=-realValues[3,jjj]*abs(prospects_b_sub.neg[,3])^realValues[1,jjj]
  wbx=prospects_b_sub.neg[,2]^realValues[2,jjj]/(prospects_b_sub.neg[,2]^realValues[2,jjj]+(1-prospects_b_sub.neg[,2])^realValues[2,jjj])^(1/realValues[2,jjj])
  wby=prospects_b_sub.neg[,4]^realValues[2,jjj]/(prospects_b_sub.neg[,4]^realValues[2,jjj]+(1-prospects_b_sub.neg[,4])^realValues[2,jjj])^(1/realValues[2,jjj])
  #vfb=bx*wbx+by*wby
  #vf=1/(1+exp(realValues[4,jjj]*((way*ay)+(wax*ax)-(wby*by)-(wbx*bx))))
  #vf=1/(1+exp(realValues[3,jjj]*(vfb-vfa)))
  
  items_n_neg=items_n[2,jjj]-items_n[1,jjj]
  #vfpos=c(vf)#,ay,bx,by)
  #rawdata_sub=vf
  ...
  player_dat  = list("prospects_a_neg"=prospects_a_sub.neg,
                     "prospects_b_neg"=prospects_b_sub.neg,
                     "N"=N,
                     "rawdata"=rawdata_sub, 
                     "items_n_neg"=items_n_neg,
  ...
  parameters = c("beta", "delta",'lambda', 'luce')
  ...
  parametersTemp<-c('beta', 'delta', 'lambda','luce')
  ...
  for( iii in 1:subjectsperrun){
    ...
    if (subjectsperrun>1){
      ...
      blockedcovarMatrix<-matrix(covariancemat[as.matrix(coordinatesOfCovarMat)],nrow=4)
    }
    if (subjectsperrun==1){
...
# .....lambda...
# .....luce.....
estimates<-matrix(nrow=4, ncol=subsetOfSubjects)
estimates[1,]<-beta[2:(subsetOfSubjects+1),1]
estimates[2,]<-delta[2:(subsetOfSubjects+1),1]
estimates[3,]<-lambda[2:(subsetOfSubjects+1),1]
estimates[4,]<-luce[2:(subsetOfSubjects+1),1]
...
for (j in 1:subsetOfSubjects){
  invCovarianceMatrix[[j]]<-solve(covariancematrix[(2+(4*(j-1))):(1+(4*j)),1:4])
  colnames(invCovarianceMatrix[[j]])<-labelsofCovarianceMat
  rownames(invCovarianceMatrix[[j]])<-labelsofCovarianceMat
  invVarianceMatrix[[j]]<-solve(diag(4)*covariancematrix[(2+(4*(j-1))):(1+(4*j)),1:4])
...
reducedChiSquare<-sum(chiSquareElement)/(4*subsetOfSubjects)
reducedChiSquare
chiSquareMatrix_Variance<-matrix(Reduce('+', chiSquareElement_Variance),4,4)
colnames(chiSquareMatrix_Variance)<-labelsofCovarianceMat
rownames(chiSquareMatrix_Variance)<-labelsofCovarianceMat
reducedChiSquare_Variance<-chiSquareMatrix_Variance%*%matrix(1,4,1)/(subsetOfSubjects-1)
...
write.table(covariancematrix,"C:\\Users\\Daniel\\Documents\\R\\Rstan_covariancemat_dataset18_neg.xls",sep="\t")
write.table(invCovarianceMatrix,"C:\\Users\\Daniel\\Documents\\R\\Rstan_invcovariancemat_dataset18_neg.xls",sep="\t")
write.table(alpha,"C:\\Users\\Daniel\\Documents\\R\\Rstan_alphaestimate_dataset18_neg.xls",sep="\t")
write.table(beta,"C:\\Users\\Daniel\\Documents\\R\\Rstan_betaestimate_dataset18_neg.xls",sep="\t")
write.table(delta,"C:\\Users\\Daniel\\Documents\\R\\Rstan_deltaestimate_dataset18_neg.xls",sep="\t")
write.table(gamma,"C:\\Users\\Daniel\\Documents\\R\\Rstan_gammaestimate_dataset18_neg.xls",sep="\t")
write.table(lambda,"C:\\Users\\Daniel\\Documents\\R\\Rstan_lambdaestimate_dataset18_neg.xls",sep="\t")
write.table(luce,"C:\\Users\\Daniel\\Documents\\R\\Rstan_luceestimate_dataset18_neg.xls",sep="\t")

allChiSquares<-matrix(c(chiSquareMatrix_Variance%*%matrix(1,4,1), sum(chiSquareElement)),5,1)
degreesOfFreedom<-rbind(matrix((subsetOfSubjects-1),4,1), 4*subsetOfSubjects)
...
write.table(allChiSquares,"C:\\Users\\Daniel\\Documents\\R\\Rstan_AllChiSquaresValues_dataset18_neg.xls",sep="\t")
