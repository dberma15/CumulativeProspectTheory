...
#             100 130 124 80
#
#   knownBeta, knownDelta, knownLambda, knownLuce - These are the known values of the parameters beta, delta, lambda, and luce where beta=(0,1), delta=(0,1), lambda=(1,5),luce=(.5,1.5)
#                Four text files must be listed in these variables
#
...
subsetOfSubjects<-10 #The total number of subjects being run in the analysis (must be less than or equal to totalsubjects). 
totalsubjects<-subsetOfSubjects-firstSubject+1 #total number of subjects stored in the data sets
numberOfParameters=4 #number of parameters being estimated (4, beta, delta, lambda, luce)
...
#   items_n_neg=items_n[1,jjj]
#   rawdata_sub=rawdata[1:items_n_neg,jjj]
#   prospects_a_sub.neg=prospects_a_sub[1:items_n_neg,]
#   prospects_b_sub.neg=prospects_b_sub[1:items_n_neg,]
...
#  prints the estimated parameters from hierarchical, digits is the number of digits it prints
  ...
  #Stores all variable estimates and basic statistics about them#Stores all six variable estimates and basic statistics about them
...
estimates[1,]<-beta[2:(totalsubjects+1),1]
estimates[2,]<-delta[2:(totalsubjects+1),1]
estimates[3,]<-lambda[2:(totalsubjects+1),1]
estimates[4,]<-luce[2:(totalsubjects+1),1]
...
if (testingKnownData==1){
  ...
  realValues[1,]=t(as.matrix(read.table(knownBeta), fill=T))[firstSubject:subsetOfSubjects]
  realValues[2,]=t(as.matrix(read.table(knownDelta), fill=T))[firstSubject:subsetOfSubjects]
  realValues[3,]=t(as.matrix(read.table(knownLambda), fill=T))[firstSubject:subsetOfSubjects]
  realValues[4,]=t(as.matrix(read.table(knownLuce), fill=T))[firstSubject:subsetOfSubjects]
